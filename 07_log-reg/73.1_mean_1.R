...
dataset_1 <- read.csv(
file = "06_multicollinearities/02_output/dataset_mean_vif_1.csv",
sep = ";"
)

print("Dataset: Mean, semester 1")
...
cm_results_mean_1_actual <- confusionMatrix(pred_actual$pred, pred_actual$obs)
print(cm_results_mean_1_actual)

print("crossvalidation model on balanced data")
cm_results_mean_1_smoted <- confusionMatrix(pred_smoted$pred, pred_smoted$obs)
print(cm_results_mean_1_smoted)

print("model coefficients on balanced data")
summary(model_1_cv_smoted) #$coef

# change model names for identification
model_mean_1_actual <- model_1_cv
model_mean_1_smoted <- model_1_cv_smoted
...
### TEST ####
# test of "higher" models on "simpler" data
...
pred_actual <- pred(model_mean_3_actual, dataset_1)
pred_smoted <- pred(model_mean_3_smoted, dataset_1)

print("crossvalidation model on actual data")
cm_results_mix_actual <- confusionMatrix(pred_actual$pred, pred_actual$obs)
print(cm_results_mix_actual)

print("crossvalidation model on balanced data")
cm_results_mix_smoted <- confusionMatrix(pred_smoted$pred, pred_smoted$obs)
print(cm_results_mix_smoted)

rm(dataset_1, backup, dataset_3, pred_actual, pred_smoted, cm_results_mix_actual, cm_results_mix_smoted)
