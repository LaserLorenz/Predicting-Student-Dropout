...
dataset_1 <- read.csv(
file = "06_multicollinearities/02_output/dataset_mean_vif_3.csv",
sep = ";"
)

print("Dataset: Mean, semester 3")
...
cm_results_mean_3_actual <- confusionMatrix(pred_actual$pred, pred_actual$obs)
print(cm_results_mean_3_actual)

print("crossvalidation model on balanced data")
cm_results_mean_3_smoted <- confusionMatrix(pred_smoted$pred, pred_smoted$obs)
print(cm_results_mean_3_smoted)

print("model coefficients on balanced data")
summary(model_1_cv_smoted) #$coef

# change model names for identification
model_mean_3_actual <- model_1_cv
model_mean_3_smoted <- model_1_cv_smoted
